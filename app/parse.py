...
class WebScraper:
    ...
    def scrape_single_page(self, url) -> [Product]:
        result = []
        self.driver.get(url)
        ...
        for product in products:
            result.append(
                Product(
                    title=product.find_element(By.CSS_SELECTOR, "a.title").get_attribute("textContent"),
                    ...
                    rating=len(
                        product.find_elements(By.CSS_SELECTOR, "p > span.ws-icon-star")
                    ),
                    num_of_reviews=int(
                        product.find_element(
